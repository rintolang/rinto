...
typedef struct {
        /* cbIdent is called when an IDENT is detected. */
        void (*cbIdent)     (std::string, position) = nullptr;

        /* cbNewfile is called when the scanner has finished reading
           a file and has moved on to the next one in queue.*/
        bool (*cbNewfile)   (std::string)           = nullptr;

        /* cbScanError is called when the scanner issues a scan error. */
        void (*cbScanError) (scanError)             = nullptr;

        /* cbEOF is called when the scanner has reached the end of
           a file.*/
        void (*cbEOF)       ()                      = nullptr;
...
private:
        ...
        /* Expect particular token/tokenTypes in the given
           order. exoect() evaluates at end of file, whereas
           expectNext() assumes that the expected tokens will
           appear in their given order immediately after
           the current token. */
